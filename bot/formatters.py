...
def format_help(is_subscribed: bool) -> str:
    ...
    return (
        ...
        "/fix — исправленный тег: выбрать непрошедший образ и прислать новый\n"
        ...
        "/version — версия бота и git sha образа (проверка деплоя GHCR)\n\n"
        "<b>Поиск командой:</b>\n"
        ...
        "статусы в таблице.\n\n"
        "Каноническая таблица — ссылка ниже; старый xls-реестр зеркалируется "
        "ботом (новые теги подтягиваются при /sync и каждый час).\n\n"
        f"{sub_state}\n"
        f"{_SHEET_LINK}"
    )
